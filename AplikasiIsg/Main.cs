    ...
    public partial class Main : MetroFramework.Forms.MetroForm
    {
        ...
        private void t_Tick(object sender, EventArgs e)
        {
            ...
            if (next < DateTime.Now)
            {
                ...
                {
                    //konversi
                    string[] files = fman.getFile("c:\\tes\\inbox\\");
                ...
                int jumOut = fman.cekDir("C:\\tes\\outbox\\");
                log = "[" + DateTime.Now.ToString("HH:MM:ss") + "]  " + jumOut.ToString() + " files on Outbox";
                updateLog(log);

                if (jumOut > 0)
                {
                    string[] files = fman.getFile("c:\\tes\\outbox\\");
                    string pes = "";
                    string[] fName = null;
                    if (files != null && files.Length > 0)
                    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            var ws = new wsClass();
            string[] pesan = fm.getFile("c:\\tes\\outbox\\");
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            var fm = new fileMan();
            var ws = new wsClass();
            string[] files = fm.getFile("c:\\tes\\outbox\\");
            string pes = "";
            string[] fName = null;
            if (files != null && files.Length>0)
            {
